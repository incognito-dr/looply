...
function standings(state, upto = Infinity) {
  ...
  for (const m of state.partidos) {
    if (m.ronda > upto || !done(m)) continue;
...
function vPartidos() {
  if (!hasT()) return '';
  const total = maxRound(), planned = S.torneo.rondas || total;
  ...
  const chips = [];
  for (let i = 1; i <= total; i++) chips.push(`<button class="rchip ${i === r ? 'on' : ''} ${roundDone(i) ? 'done' : ''}" data-act="ronda" data-r="${i}" aria-label="Ronda ${i}"><span>R${i}</span></button>`);
  for (let i = total + 1; i <= planned; i++) chips.push(`<span class="rchip" style="opacity:.35;border-style:dashed" aria-hidden="true">R${i}</span>`);
  const pts = S.torneo.puntos;
  const courts = ms.map(m => {
    ...
    return `<article class="court">
      <span class="court-tag">Cancha ${m.cancha}</span>
  ...
  let next = '';
  if (S.torneo.formato === 'mexicano' && r === total && roundDone(r) && total < planned) {
    next = `<button class="btn primary wide next-round" data-act="nextRound">Armar ronda ${total + 1}</button>
    ...
    next = `<button class="btn wide next-round" data-act="ronda" data-r="${r + 1}">Ir a la ronda ${r + 1}</button>`;
  } else if (allDone()) {
  ...
  return `
    ...
    <div class="round-head">
      <div><h1 class="h1">Ronda ${r}</h1><p class="sub" style="margin:4px 0 0">de ${planned} · partidos a ${pts ? pts + ' puntos' : 'puntos libres'}</p></div>
      ${timerChip()}
    </div>
...
const allDone = () => hasT() && maxRound() >= (S.torneo.rondas || 0) && S.partidos.every(done);

function vTabla() {
  if (!hasT()) return '';
  const st = standings(S);
  const played = [...new Set(S.partidos.filter(done).map(m => m.ronda))];
  const L = played.length ? Math.max(...played) : 0;
  const prev = L > 1 ? new Map(standings(S, L - 1).map(x => [x.n, x.pos])) : null;
  const doneN = S.partidos.filter(done).length, totN = S.partidos.length;
  const expected = S.torneo.formato === 'mexicano' ? Math.round(totN / maxRound() * (S.torneo.rondas || maxRound())) : totN;
  ...
  return `
    ...
    <div class="panel">
      <dl class="kv">
        ...
        <dt>Rondas</dt><dd class="num">${maxRound()} de ${t.rondas}</dd>
...
function defaultForm() {
  ...
  return { nombre: 'Americano Brositos', formato: 'americano', jugadores: prevNames, canchas: Math.max(1, Math.floor(n / 4)), puntos: 24, rondas: suggestRounds(n, Math.max(1, Math.floor(n / 4)), 'americano'), rondasTocadas: false, clave: '' };
}
function vForm() {
  const f = ui.form || (ui.form = defaultForm());
  const n = f.jugadores.length, maxC = Math.max(1, Math.floor(n / 4));
  if (f.canchas > maxC) f.canchas = maxC;
  if (!f.rondasTocadas) f.rondas = suggestRounds(n, f.canchas, f.formato);
  const per = Math.min(f.canchas, Math.floor(n / 4)) * 4, rest = n - per;
  let note = '';
  if (n < 4) note = `Faltan ${4 - n} para la primera cancha.`;
...
function sScore() {
  ...
  const P = S.torneo.puntos, max = P || 30, cur = d['p' + d.side];
  ...
  return `
    <p class="sheet-title">Ronda ${m.ronda} · Cancha ${m.cancha}</p>
    <p class="sheet-sub">${P ? `Toca los puntos de una pareja; la otra se completa hasta ${P}.` : 'Toca una pareja y marca sus puntos.'}</p>
    <div class="dual">
    ...
    <div class="sheet-actions">
      ...
      <button class="btn primary" data-act="saveScore" ${Number.isFinite(d.pa) && Number.isFinite(d.pb) ? '' : 'disabled'}>Guardar resultado</button>
    </div>`;
}
...
function sPlayer() {
  ...
  const items = h.map(({ m, partner, opp, f, c, d }) => {
    ...
    return `<li><span class="r">R${m.ronda}</span><span>con <b>${esc(partner)}</b><br><small style="color:var(--muted)">contra ${esc(opp[0])} y ${esc(opp[1])}</small></span>${res}</li>`;
  }).join('');
  return `
...
document.addEventListener('click', e => {
  ...
  switch (act) {
    ...
    case 'pick': {
      const d = ui.draft, v = +el.dataset.v, P = S.torneo.puntos, o = d.side === 'a' ? 'b' : 'a';
      d['p' + d.side] = v;
      if (P) d['p' + o] = P - v;
    ...
    case 'fset': {
      const k = el.dataset.k; f[k] = k === 'puntos' ? +el.dataset.v : el.dataset.v;
      if (k === 'formato') f.rondasTocadas = false;
      render(); break;
    }
    case 'fstep': {
      const k = el.dataset.k, d = +el.dataset.d, n = f.jugadores.length;
      if (k === 'canchas') { f.canchas = Math.min(Math.max(1, f.canchas + d), Math.max(1, Math.floor(n / 4))); f.rondasTocadas = false; }
      else { f.rondas = Math.min(Math.max(1, f.rondas + d), 30); f.rondasTocadas = true; }
      render(); break;
    }
    case 'delPlayer': f.jugadores.splice(+el.dataset.i, 1); f.canchas = Math.max(1, Math.floor(f.jugadores.length / 4)); render(); break;
...
document.addEventListener('submit', async e => {
  ...
  if (kind === 'addPlayer') {
    ...
    f.canchas = Math.max(1, Math.floor(f.jugadores.length / 4));
